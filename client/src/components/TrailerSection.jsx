    ...
    <div className='px-6 md:px-16 lg:px-24 xl:px-44 py-20 overflow-hidden'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto'>
            {dummyTrailers.map((trailer) => (
                <div
                    key={trailer.image}
                    className='relative hover:opacity-100 opacity-80 hover:-translate-y-1 duration-300 transition h-40 cursor-pointer group'
                    onClick={() => setCurrentTrailer(trailer)}
                >
                    <img
                        src={trailer.image}
                        alt=""
                        className='rounded-lg w-full h-full object-cover brightness-75 group-hover:brightness-110 transition duration-300'
                    />
                    <PlayCircleIcon
                        strokeWidth={1.6}
